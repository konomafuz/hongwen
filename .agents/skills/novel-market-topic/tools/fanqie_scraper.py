...
爬取番茄小说 API，获取女频各分类排行榜数据，输出结构化 JSON。

使用方法：
    python fanqie_scraper.py                              # 默认爬取所有分类日榜，输出到 stdout
    python fanqie_scraper.py -o data.json                 # 输出到文件
    python fanqie_scraper.py --categories 古代言情,现代言情  # 指定分类
...
def discover_categories() -> dict[str, str]:
    ...
    for url in discovery_urls:
        params = {"app_id": APP_ID, "gender": "female"}
            ...
            if isinstance(items, list):
                for item in items:
                    ...
                    if name and cid:
                        categories[name] = cid
            if categories:
                log(f"自动发现 {len(categories)} 个分类：{', '.join(categories.keys())}")
                return categories
        sleep_between_requests()

    log("自动发现分类失败，使用内置分类列表")
...
def scrape_category(
    ...
    for page in range(pages):
        ...
        raw_list = fetch_rank_page(category_id, rank_mold, offset)
...
def scrape_all(
    ...
    for i, (cat_name, cat_id) in enumerate(categories.items()):
        books = scrape_category(cat_name, cat_id, rank_type, pages)
